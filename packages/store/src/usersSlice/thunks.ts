...
const combinePermissions = (existingPermissions: CombinedPermissions, additionalPermissions: CombinedPermissions = {}) =>
    ...
    .then(({ data }) => {
      const permissionSets = data.reduce(
        (accu, permissionSet) => {
          ...
          let permissionSetObject: PermissionSet = { ...permissionSetState, ...permissionSet };
          ...
          const scopes = Object.values(scopedPermissionAreas).reduce((accu: UiPermissionsByAreaKey[], { key, scopeType }) => {
          ...
          permissionSetObject = scopes.reduce((accu, scope) => {
            accu.result[scope] = mapPermissionSet(permissionSetObject.name, [scopedPermissionAreas[scope].excessiveAccessSelector], scope) as any;
            return accu;
          }, permissionSetObject);
      ...
      return Promise.all([dispatch(actions.receivedPermissionSets(permissionSets)), permissionSets]) as any;
    })

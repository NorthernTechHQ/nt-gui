...
export const getCurrentCard = createAppAsyncThunk(`${sliceName}/getCurrentCard`, (_, { dispatch }) =>
  Api.get<BillingInfo>(`${tenantadmApiUrlv2}/billing`).then(({ data: { card } }) => {
    if (card) {
...
const prepareAuditlogQuery = ({
  ...
  const detail = typeof detailFilter === 'string' ? detailFilter : detailFilter?.id;
...
type CreateTenantPayload = {
  ...
  name: string;
  sso: boolean;
  users?: [{ email: string; role: (typeof rolesByName)['admin'] }];
};

export const addTenant = createAppAsyncThunk(`${sliceName}/createTenant`, (selectionState: CreateTenantPayload, { dispatch }) =>
...
// eslint-disable-next-line @typescript-eslint/no-unused-vars
export const createIntegration = createAppAsyncThunk(`${sliceName}/createIntegration`, ({ id, ...integration }: { id: string } & Integration, { dispatch }) =>
...
export const changeIntegration = createAppAsyncThunk(`${sliceName}/changeIntegration`, ({ id, credentials }: { id: string } & Integration, { dispatch }) =>
...
export const deleteSsoConfig = createAppAsyncThunk(`${sliceName}/deleteSsoConfig`, ({ id }: { id: string }, { dispatch, getState }) =>
...
export const getSsoConfigById = createAppAsyncThunk(`${sliceName}/getSsoConfigById`, (ssoConfig: SSOConfig, { dispatch }) =>
  Api.get(`${ssoIdpApiUrlv1}/${ssoConfig.id}`)
    ...
    .then(({ data, headers }) => {
      ...
      return sso ? Promise.resolve({ ...ssoConfig, config: data, type: sso.id }) : Promise.reject('Unsupported SSO config content type.');
    })
);
...
export const getSsoConfigs = createAppAsyncThunk(`${sliceName}/getSsoConfigs`, (_, { dispatch }) =>
  Api.get<SSOConfig>(ssoIdpApiUrlv1)
    ...
    .then(({ data }) =>
      Promise.all(data.map((ssoConfig: SSOConfig) => dispatch(getSsoConfigById(ssoConfig)).unwrap()))

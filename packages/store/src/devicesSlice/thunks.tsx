...
import { StatusDeviceauth } from '@northern.tech/store/api/types/StatusDeviceauth';
...
import { Device, DeviceFilter, DeviceGroup, DeviceGroups, DeviceListState, DeviceSelectedAttribute, actions, sliceName } from '.';
import {
  ALL_DEVICE_STATES,
  DEVICE_STATES,
...
type ReducedDevice = DeviceDeviceauth & Omit<BackendDevice, 'status'> & DeviceReporting;
const reduceReceivedDevices = (devices: ReducedDevice[], ids: string[], state: RootState, status?: DeviceDeviceauth.status | undefined) =>
  devices.reduce(
    (accu, device: any) => {
    ...
    Promise.resolve(
      dispatch(
        getDevicesByStatus({
          ...
          filterSelection,
          group,
          status: shouldIncludeAllStates ? undefined : (DEVICE_STATES.accepted as DeviceDeviceauth.status)
        })
      )
    )
...
/*
  ...
  */
export const getDeviceCount = createAppAsyncThunk(`${sliceName}/getDeviceCount`, (status: DeviceDeviceauth.status, { dispatch, getState }) =>
  GeneralApi.post(getSearchEndpoint(getState()), {
    page: 1,
    per_page: 1,
...
export const getAllDeviceCounts = createAppAsyncThunk(`${sliceName}/getAllDeviceCounts`, (_, { dispatch }) =>
  Promise.all([DEVICE_STATES.accepted, DEVICE_STATES.pending].map(status => dispatch(getDeviceCount(status as DeviceDeviceauth.status))))
);
...
export const setDeviceListState = createAppAsyncThunk(
  ...
  ({ shouldSelectDevices = true, forceRefresh, fetchAuth = true, ...selectionState }: SetDeviceListStatePayload, { dispatch, getState }) => {
    ...
    if (!nextState.setOnly && !deepCompare(currentRequestState, nextRequestState)) {
      ...
      tasks.push(
        dispatch(getDevicesByStatus({ ...nextState, status: applicableSelectedState as DeviceDeviceauth.status, sortOptions: sortBy, fetchAuth }))
          .unwrap()
          .then(results => {
            const { deviceAccu, total } = results[results.length - 1];
...
type GetDevicesByStatusPayload = {
  ...
  status: DeviceDeviceauth.status | undefined;
};
// get devices from inventory
export const getDevicesByStatus = createAppAsyncThunk(`${sliceName}/getDevicesByStatus`, (options: GetDevicesByStatusPayload, { dispatch, getState }) => {
  const {
    status,
    fetchAuth = true,
    filterSelection,
...
export const getAllDevicesByStatus = createAppAsyncThunk(`${sliceName}/getAllDevicesByStatus`, (status: DeviceDeviceauth.status, { dispatch, getState }) => {
...
export const getReportsDataWithoutBackendSupport = createAppAsyncThunk(`${sliceName}/getReportsDataWithoutBackendSupport`, (_, { dispatch, getState }) =>
  Promise.all([dispatch(getAllDevicesByStatus(DEVICE_STATES.accepted as DeviceDeviceauth.status)), dispatch(getGroups()), dispatch(getDynamicGroups())]).then(
    () => {
...
type GetSessionDetails = { deviceId: string; endDate: string; sessionId: string; startDate: string; userId: string };
export const getSessionDetails = createAppAsyncThunk(
  `${sliceName}/getSessionDetails`,
  ({ sessionId, deviceId, userId, startDate, endDate }: GetSessionDetails) => {
...
type SetDeviceTwinType = { deviceId: string; integration: Integration; settings: any };
export const setDeviceTwin = createAppAsyncThunk(
  `${sliceName}/setDeviceTwin`,
  ({ deviceId, integration, settings }: SetDeviceTwinType, { dispatch, getState }) =>

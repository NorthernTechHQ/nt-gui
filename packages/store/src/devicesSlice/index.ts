...
import { DEVICE_STATES, DeviceStateValue } from './constants';
...
export type DeviceStatus = DeviceStateValue | 'active' | 'inactive';
...
export type DeviceSliceType = {
  ...
  byStatus: Record<DeviceStatus, { deviceIds: string[]; total: number | null }>;
...
export const devicesSlice = createSlice({
  ...
  reducers: {
    ...
    setDevicesByStatus: (
      state,
      action: PayloadAction<{ deviceIds: string[]; forceUpdate?: boolean; status: DeviceDeviceauth.status; total: number | null }>
    ) => {
    ...
    setDevicesCountByStatus: (state, action: PayloadAction<{ count: number; status: DeviceDeviceauth.status }>) => {
    ...
    maybeUpdateDevicesByStatus: (state, action: PayloadAction<{ authId: string; deviceId: string }>) => {
      ...
      if (!hasMultipleAuthSets && (Object.values(DEVICE_STATES) as string[]).includes(device.status)) {
        ...
        state.byStatus[device.status] = { deviceIds, total: Math.max(0, (state.byStatus[device.status].total || 0) - 1) };
      }
    }
  }
});

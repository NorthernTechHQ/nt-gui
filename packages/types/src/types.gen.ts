...
export type DeploymentV1 = {
   ...
   *
   * For a deployment created with an explicit list of `phases`, this is
   * that list together with the current state of every phase.
   *
   * For a deployment created with a `uniform_phases` configuration, this
   * is the list of phases generated from that configuration.
   *
   */
 ...
 *
 * * `pending` - the phase has not started yet; its `start_ts` is in the
 * future and no device has been assigned to it.
 * * `inprogress` - the phase has started and is still accepting devices;
 * fewer devices than the phase batch size have requested an update.
 * * `finished` - the phase is complete; either its batch size has been
 * reached, or the deployment finished or was aborted before the phase
 * could accept more devices.
 *
 */
export const DeploymentPhaseStatus = {
  /**
   * DEPLOYMENT_PHASE_STATUS_PENDING
   */
  DEPLOYMENT_PHASE_STATUS_PENDING: 'pending',
  /**
   * DEPLOYMENT_PHASE_STATUS_INPROGRESS
   */
  DEPLOYMENT_PHASE_STATUS_INPROGRESS: 'inprogress',
  /**
   * DEPLOYMENT_PHASE_STATUS_FINISHED
   */
  DEPLOYMENT_PHASE_STATUS_FINISHED: 'finished'
} as const;
...
export type DeploymentPhaseSpec = {
   ...
   * Phase identifier.
   *
   * For a phase generated from a `uniform_phases` configuration, this is
   * the 1-based position of the phase in the rollout, as a string
   * (`"1"`, `"2"`, ...).
   *
   */
  id?: string;
   ...
   *
   * For a phase generated from a `uniform_phases` configuration, this is
   * always set: the configured `start_ts` plus the configured
   * `time_interval` multiplied by the number of preceding phases.
   *
   */
  start_ts?: string;
  /**
 ...
 * every `time_interval`.
 *
 * Only this configuration is submitted when creating a deployment; the
 * phases themselves are generated by the server. A deployment object
 * returned by the API carries both the configuration and the generated
 * phases, in the deployment's `phases` field.
 *
 */
...
export type SearchInventoryByIdentityData = {
  /**
   * Identity search parameters
   */
  body?: SearchIdentityParams;
  path?: never;
  query?: never;
  url: '/api/management/v2alpha1/inventory/identities/search';
};

export type SearchInventoryByIdentityErrors = {
  /**
   * Invalid Request.
   */
  400: Error;
...
export type SearchInventoryByIdentityError = SearchInventoryByIdentityErrors[keyof SearchInventoryByIdentityErrors];

export type SearchInventoryByIdentityResponses = {
  /**
   * ListOfDevices
   *
   * Successful response.
...
export type SearchInventoryByIdentityResponse = SearchInventoryByIdentityResponses[keyof SearchInventoryByIdentityResponses];
